                    ...
                    if (!StringUtils.isNullOrEmpty(msgType)) {
                        switch (msgType) {
                            case NotifyType.APP_NOTICE:
                                msgpusher.pushRoleMsgToUserList((List<String>) resultMap.get("receiverList"), "Message", "Notice");
                                break;
                            case NotifyType.APP_ROLE:
                            case NotifyType.APP_PROJECT:
                                msgpusher.pushRoleMsgToUserList((List<String>) resultMap.get("receiverList"),"Permission", "Operator");
                                break;
        ...
        try {
            if (!StringUtils.isNullOrEmpty(m.getMessageType())) {
                switch (m.getMessageType()) {
                    case NotifyType.APP_NOTICE:
                        msgpusher.pushRoleMsgToUserList(m.getReceiverList(), "Message", "Notice");
                        break;
                    case NotifyType.APP_ROLE:
                    case NotifyType.APP_PROJECT:
                        msgpusher.pushRoleMsgToUserList(m.getReceiverList(),"Permission", "Operator");
                        break;
                }
            }
        } catch (Exception e) {
